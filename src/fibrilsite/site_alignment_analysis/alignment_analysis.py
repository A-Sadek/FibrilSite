...
def align_site_pcd(df:pd.DataFrame, ply_files:list, input_feats_npy:list, output:str):
    ...
    for idx00 in tqdm(df.index, desc="aligning sites"):
        ...
        src_ply    = [p for p  in ply_files if src_pocket.split("_")[-1]    in p][0]
        target_ply = [p for p  in ply_files if target_pocket.split("_")[-1] in p][0]
...
def align_site_fibrils(align_out:str, src_pockets_paths:list, ply_files:list, fibrils_paths:list, fibril_pocket_map:dict, reg_results_paths:list):
    ...
    for file in tqdm(sorted([os.path.abspath(p).strip() for p in glob.iglob(os.path.join(align_out, "*"))]), desc="aligning fibrils"):
        ...
        pocket01_start_xyz = [p.strip() for p in src_pockets_paths if pocket01_name in p][0]
        pocket02_start_xyz = [p.strip() for p in src_pockets_paths if pocket02_name in p][0]
        ...
        pocket01_pcd_path = [p for p in ply_files if pocket01_name in p][0]
        pocket02_pcd_path = [p for p in ply_files if pocket02_name in p][0]
        ...
        reg_npy = [p for p in reg_results_paths if pocket01_name in p if pocket02_name in p][0]
        reg_results = np.load(reg_npy, allow_pickle=True).item()['icp_test']
